...
<jsp:useBean id="getRSCount" class="com.dao.ScriptDAO" scope="request"/>
<html>
<head>
...
<script >
function createNewScrip(id){	
	var newScrip="<div id='scrip"+id+"' class='Style"+scriptForm.color.value+"' style='left:<wghRandom:createRandom n="920" base="5"/>px;top:<wghRandom:createRandom n="376" base="184"/>px;z-index:"+iLayerMaxNum+"' onmousedown='Move(this,event)' ondblclick=\"Show("+id+",'shadeDiv')\"><p class='Num'>字条编号："+id+"&nbsp;&nbsp;人气：<span id='hitsValue"+id+"'>0</span><img src='images/close.gif' alt='关闭' onclick='myClose("+id+")'></p><br /><p class='Detail'><img src='images/face/face_"+scriptForm.face.value+".gif'><span class='wishMan'>"+scriptForm.wishMan.value+"</span>	<br />"+scriptForm.content.value+"</p><p class='wellWisher'>"+scriptForm.wellWisher.value+"</p><p class='comment'><a href='#' onclick='holdout("+id+")'>[支持]</a></p><p class='Date'>"+getTime()+"</p></div>";
...
<div id="main">
	...
	<div id="scrip<s:property value="id" />" class="Style<s:property value="id" />" style="left:<wghRandom:createRandom n="920" base="5"/>px;top:<wghRandom:createRandom n="376" base="184"/>px; z-index:<s:property value="id" />" onmousedown="Move(this,event)" ondblclick="Show(<s:property value="id" />,'shadeDiv')">
		<p class='Num'>字条编号：<s:property value="id" />&nbsp;&nbsp;人气：<span id="hitsValue<s:property value="id" />"><s:property value="hits" /></span><img src='images/close.gif' alt='关闭' onClick="myClose(<s:property value="id" />)"></p>
		<br /> 
		<p class="Detail">
		<img src="images/face/face_<s:property value="face" />.gif">
		<span class='wishMan'><s:property value="wishMan" /></span>
		<br />
		<s:property value="content" /></p>
		<p class='wellWisher'><s:property value="wellWisher" /></p>
		<p class="comment"><a href="#" onClick="holdout(<s:property value="id"/>)">[支持]</a></p>
		<p class="Date"><s:property value="sendTime" /></p>
	</div>
	</s:iterator>
</div>
<!--显示字条信息结束-->
